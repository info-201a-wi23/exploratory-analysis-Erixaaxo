...
KB_data_df <- kidz_bop_df %>% group_by(badword, ogArtist, category, year) %>% summarize(total_count = sum(count))

# Made a variable for how many rows in the new df
num_categories <- nrow(KB_data_df)

# Created a separate dataframe to count up how many times the categories
category_count <- data.frame(KB_data_df$category)
...
summary_table <- summary_table[,c("badword", "ogArtist", "total_count", "heyear", "category", "category_count", "category_prop")]
View(summary_table)
